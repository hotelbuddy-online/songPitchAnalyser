...
program
  ...
  .option(
    "-d, --debug [type]",
    "0: none, 1: Output power spectrum, post-filterbank values, and Mel coefficients. 2: output filter banks used. 3: output frequency magnitudes. Default 0.",
    0
  )
  .option("-w, --wav [wav]", "Wave file path to process.", undefined)
  .option(
    "-m, --minFrequency [number]",
    "Low frequency cutoff for MFCC. Default 300",
    300
  )
  .option(
    "-x, --maxFrequency [number]",
    "High frequency cutoff for MFCC. Default 3500",
    3500
  )
  .option(
    "-f, --numMelSpecFilters [number]",
    "Number of mel spec filter banks to use. Default is 26.",
    26
  )
  .option(
    "-n, --samplesPerFrame [number]",
    "Number of samples per frame to pass into the FFT. Default 128.",
    128
  )
  .option(
    "-s, --samplesPerStep [number]",
    "Number of samples to step between each frame. Default is samplesPerFrame.",
    128
  );
...
if (program.samplesPerFrame & (program.samplesPerFrame - 1 !== 0))
  throw Error(
    "Please provide a samplesPerFrame that is a power of 2 (e.g. 32, 64, 128, 256, etc.). Was: " +
      program.samplesPerFrame
  );
...
wr.on("data", function (buffer, offset, length) {
  framer.frame(buffer, function (frame, fIx) {
    ...
    if (program.debug == 1) {
      ...
      console.log(
        "Post-filters(" +
          result.melSpec.length +
          "): " +
          result.melSpec.join(",")
      );
      console.log(
        "Post-filters Log(" +
          result.melSpecLog.length +
          "): " +
          result.melSpecLog.join(",")
      );
...
wr.on("format", function (format) {
  ...
  ham = require("signal-windows").windows.construct(
    "ham",
    program.samplesPerFrame
  );

  var ulawMap = format.ulaw
    ? JSON.parse(fs.readFileSync("data/ulaw2pcm.json").toString())
    : undefined;
  ...
  if (format.channels != 1)
    throw Error(
      "Right now this MFCC code only works on single channel 8-bit wave files."
    );
